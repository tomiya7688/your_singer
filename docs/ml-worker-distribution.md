...
pyopenjtalk 0.4.1の日本語辞書はパッケージ導入直後には存在せず、初回G2P時に公式OpenJTalk v1.11.1資源を取得する。利用者環境でこの取得を発生させないため、配布ビルド時にG2Pを一度実行して辞書を展開し、その実ファイルをPyInstallerへ含める。包装後のpreflightで辞書ディレクトリの存在も検証する。
