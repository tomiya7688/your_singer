...
public sealed class AutoCorrectionService
{
    ...
    {
        var dataset = await new TrainingDatasetSnapshotService(_datasetRepository)
        ...
        var fingerprint = TrainingDatasetSnapshotService.CreateFingerprint(dataset, settings.Enabled, corrections);
    ...
    private static List<CorrectionRecord> BuildObservedOnly(UniversalVoiceDatasetRecord dataset) =>
        ...
        {
            SegmentId = x.SegmentId, State = CorrectionState.Observed, Method = "observed-only",
            Confidence = 1.0, Reason = "自動補完・補正OFF"
        }).ToList();
    ...
    private static List<CorrectionRecord> BuildCorrections(UniversalVoiceDatasetRecord dataset)
    {
        ...
        foreach (var segment in dataset.Segments)
        {
            ...
            {
                state = CorrectionState.Rejected; method = "confidence-outlier-rejection";
                confidence = 0.90; reason = "ASRまたはalignment confidenceが低いため除外";
            }
            else if (segment.PitchReliability < 0.40 && segment.ContentType == SegmentContentType.Singing)
            {
                state = CorrectionState.Corrected; method = "pitch-condition-interpolation";
                confidence = 0.65; reason = "歌唱pitch reliabilityが低いため条件間補間対象";
            }
            else if (segment.AsrConfidence < 0.65 || segment.AlignmentConfidence < 0.65)
            {
                state = CorrectionState.WeakObserved; method = "weak-observation-assist";
                confidence = 0.70; reason = "観測値を保持しつつ補助推定対象";
            }
        ...
        var counts = dataset.Segments.SelectMany(x => x.Phonemes).GroupBy(x => x.Phoneme, StringComparer.Ordinal)
        ...
        foreach (var segment in dataset.Segments)
            foreach (var phoneme in segment.Phonemes)
            {
                if (phoneme.Confidence < 0.35)
                    result.Add(new CorrectionRecord
                    {
                        SegmentId = segment.SegmentId, Phoneme = phoneme.Phoneme,
                        State = CorrectionState.Corrected, Method = "phoneme-label-confidence-correction",
                        Confidence = 0.60, OriginalValue = phoneme.Phoneme, CorrectedValue = phoneme.Phoneme,
                    ...
                    {
                        SegmentId = segment.SegmentId, Phoneme = phoneme.Phoneme,
                        State = CorrectionState.WeakObserved, Method = "cross-phoneme-speaker-feature-estimation",
                        Confidence = 0.55, Reason = "少量音素のため観測値を保持しつつ他音素から話者特徴を補助推定"
                    });
            }
        return result;
    }
